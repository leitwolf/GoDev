...
def read_config():
    ...
    if not os.path.exists(config_file):
        module_pane.console_output("Config file "+config_file+" not exists!")
        return None
    ...
    try:
        ...
        module_pane.console_output("There must be a section [mysql] in "+config_file)
        return None
    ...
    if parser.has_section("mysql"):
        ...
        ret["db"]=db
        # model所放位置，相对于项目目录
        ret["models_place"]=path+"/"+parser.get(section,"models_place",fallback="models")
    return ret
...
def handle_mysql(host,port,user,password,db):
    ...
    # 查询表列表
    cursor.execute("select table_name from information_schema.tables where table_schema='"+db+"' and table_type='base table'")
    table_names=[]
    for row in cursor:
    ...
    for table_name in table_names:
        ...
        cursor.execute("select COLUMN_NAME,DATA_TYPE,CHARACTER_MAXIMUM_LENGTH,NUMERIC_PRECISION,NUMERIC_SCALE,EXTRA from information_schema.columns where table_schema='"+db+"' and table_name='"+table_name+"'")
        column_len=0
        type_len=0
        columns=[]
        valid=True
        for t in cursor:
            ...
            # 找不到对应的类型
            if type1=="":
                module_pane.console_output("[ERRO] orm: data type ("+data_type+") not found!")
                valid=False
                break
            ...
            if data_type=="decimal":
                comment+=';digits('+str(precision)+');decimals('+str(scale)+')'
...
class GodevOrmBeegoCommand(sublime_plugin.WindowCommand):
    ...
    def do_mysql(self):
        ...
        for t in tables:
            ...
            for i in range(0,l):
                row=rows[i]
                s="    %-"+str(column_len)+"s %-"+str(type_len)+"s %s"
                # print(s)
