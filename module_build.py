...
class GodevBuildCommand(sublime_plugin.WindowCommand):
    ...
    def run(self, goos, goarch):
        ...
        t = threading.Thread(
            target=self.doit, args=(common.goroot, goos, goarch))
        t.start()
    ...
    def doit(self, goroot, goos, goarch):
        ...
        if not os.path.exists(toolpath):
            ...
            module_pane.console_output(
                "Start building toolchain for [" + platform + "] ...")
        ...
        # 开始构建
        module_pane.console_output("Start building [" + name + "] ...")
        cmd = "CGO_ENABLED=0 GOOS=" + goos + \
            " GOARCH=" + goarch + " go build " + name
        gopath = common.gopath
        env = {"GOPATH": gopath}
        args = {"cmd": cmd,
                "shell": True,
